...
def _column_exists(d, key):
    for i in d.keys():
        if key in i.lower() or key == i.lower():
            return True
    return False

def _get_column_vals(d, key):
    for k, v in d.items():
        if key in k.lower() or key == k.lower():
            return d[k]["values"]
    return None
...
def _get_age(columns):
    ...
    # Check for keys in this respective order. Quit on first match
    age_keys = ["age", "year", "yrbp"]
    age = []
    try:
        # Step 1: Look for exact key matches (case insensitive)
        for age_key in age_keys:
            # Look for existence
            if _column_exists(columns, age_key):
                # This key exists, save the values
                age = _get_column_vals(columns, age_key)
        ...
        # Step 3: No year or age found, start searching for a loose match with "age" or "year" somewhere in the variableName.
        if not age:
            # Loop through column variableNames
            for k, v in columns.items():
                k_low = k.lower()
        ...
        if age:
            ...
            for var, col in table["columns"].items():
                ...
                if "age" in var or "year" in var:
                    ...
                    #logger_inferred_data.info("get_inferred_data_table: Not calculating inferred data for variableName: {}".format(var))
                    pass
